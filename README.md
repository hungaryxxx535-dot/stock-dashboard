...
真实持仓默认写入浏览器 IndexedDB。首次打开为匿名 Demo；持仓中心按 A股、美股、港股分区展示，新增持仓统一从“上传持仓截图”进入。导入时先选择市场，可连续上传多张券商截图；OCR 在当前浏览器运行，识别结果经逐条核对和一次确认后才写入本机，截图之外的现有持仓不会自动删除。每次确认导入前都会创建可恢复快照。
