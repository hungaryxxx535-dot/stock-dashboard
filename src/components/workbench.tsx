      ...
      {sections.map(({ market, accent }) => {
        ...
        return <section key={market} aria-labelledby={`portfolio-${market}`} className={`rounded-2xl border border-t-4 border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-5 ${accent}`}>
          ...
          {!items.length ? <div className="rounded-xl border border-dashed border-slate-300 p-5 text-center dark:border-slate-700"><p className="font-bold">尚未导入{marketLabel(market)}持仓</p><p className="mt-1 text-sm text-slate-500">上传券商持仓截图即可识别，不需要逐项填写。</p></div> : <>
            <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[760px] text-left text-sm"><thead className="text-slate-500"><tr><th className="py-2">标的</th><th>数量</th><th>券商成本</th><th>经济成本</th><th>折算市值</th><th>数据口径</th><th></th></tr></thead><tbody>{items.map((item) => { const provenance = describeQuote(item.quote); return <tr key={item.holding.id} className="border-t border-slate-100 dark:border-slate-800"><td className="py-3 font-bold">{isNameOnlySymbol(item.instrument.symbol) ? item.instrument.name : item.instrument.symbol}<span className="block font-normal text-slate-500">{isNameOnlySymbol(item.instrument.symbol) ? "代码待匹配" : item.instrument.name}</span></td><td>{item.holding.quantity.toLocaleString("zh-CN")}</td><td>{nativeMoney(item.holding.brokerCost, item.instrument.currency)}</td><td>{nativeMoney(item.holding.economicCost, item.instrument.currency)}</td><td>{money(item.valueBase)}</td><td><span className={provenance.tone === "good" ? "text-emerald-700" : provenance.tone === "danger" ? "text-red-600" : "text-amber-700"}>{provenance.label}</span><span className="block max-w-56 text-xs text-slate-500">{provenance.detail}</span></td><td><button className="text-red-600" onClick={() => remove(item.holding.id)}>关闭</button></td></tr>; })}</tbody></table></div>
            <div className="space-y-3 md:hidden">{items.map((item) => { const provenance = describeQuote(item.quote); return <article key={item.holding.id} className="rounded-xl border border-slate-200 p-3 dark:border-slate-700"><div className="flex items-start justify-between gap-3"><div><p className="font-black">{isNameOnlySymbol(item.instrument.symbol) ? item.instrument.name : item.instrument.symbol}</p><p className="text-sm text-slate-500">{isNameOnlySymbol(item.instrument.symbol) ? "代码待匹配" : item.instrument.name}</p></div><button className="text-sm font-bold text-red-600" onClick={() => remove(item.holding.id)}>关闭</button></div><dl className="mt-3 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-xs text-slate-500">数量</dt><dd className="font-bold">{item.holding.quantity.toLocaleString("zh-CN")}</dd></div><div><dt className="text-xs text-slate-500">券商成本</dt><dd className="font-bold">{nativeMoney(item.holding.brokerCost, item.instrument.currency)}</dd></div><div><dt className="text-xs text-slate-500">经济成本</dt><dd className="font-bold">{nativeMoney(item.holding.economicCost, item.instrument.currency)}</dd></div><div><dt className="text-xs text-slate-500">折算市值</dt><dd className="font-bold">{money(item.valueBase)}</dd></div></dl><p className={`mt-3 text-xs ${provenance.tone === "good" ? "text-emerald-700" : provenance.tone === "danger" ? "text-red-600" : "text-amber-700"}`}>{provenance.label} · {provenance.detail}</p></article>; })}</div>
          </>}
        </section>;
      })}
    </div>
